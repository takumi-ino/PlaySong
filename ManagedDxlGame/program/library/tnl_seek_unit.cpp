...
namespace tnl {
	...
	//-------------------------------------------------------------------------------------------------------
	void SeekUnit::update(const double deltaTime) {
		if (!is_playing_) return;
		uint32_t frame = getSeekFrame(eFrameType::CURRENT);
		seekProcess(static_cast<float>(deltaTime));
		if (frame_functions_storage_.empty()) return;
	...
	//-------------------------------------------------------------------------------------------------------
	void SeekUnit::seekProcess(const float deltaTime) {

		double d_delta_time = static_cast<double>(deltaTime);
