...
*****************************************************************************************************/
	...
	public:
		...
		#define TNL_SEQ_CO_FRM_YIELD_RETURN( lim_frame, deltaTime, logic )		if (tnl_sequence_._co_yield_by_frame_( lim_frame, deltaTime, logic ) ) return true ;
		...
		// ....... フレーム数カウントは int32_t を超えると 0 にリセットされることに注意
		#define TNL_SEQ_CO_TIM_YIELD_RETURN( lim_time, deltaTime, logic )		if (tnl_sequence_._co_yield_by_time_( lim_time, deltaTime, logic ) ) return true ;
		...
		inline bool		_co_yield_by_frame_(int32_t limit_frame, float deltaTime, const std::function<void()>& logic) {
		...
		inline bool		_co_yield_by_time_(float limit_time, float deltaTime, const std::function<void()>& logic) {
			if (co_call_count_++ == co_call_through_) {
